...
import { FileText, Mail, ArrowLeft, CheckCircle, Copy } from 'lucide-react';
            ...
            {!resetUrl ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-2 text-green-600">
                  ...
                  <span className="text-sm font-medium">重置链接已生成</span>
                </div>

                <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-xs text-blue-600 mb-2">请复制以下链接在浏览器中打开：</p>
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      onClick={handleCopy}
                      className="px-3 py-1.5 bg-blue-600 text-white rounded text-xs hover:bg-blue-700 flex items-center space-x-1"
                    >
                      {copied ? <span>已复制</span> : <><Copy className="h-3 w-3" /><span>复制</span></>}
                    </button>
                  </div>
                </div>
