...
async function seedViaAPI() {
  console.log('开始通过API添加军队采购规则...\n');
  
  for (const rule of armyProcurementRules) {
    try {
      const res = await fetch('http://localhost:3000/api/admin/industry-rules', {
        method: 'POST',
      ...
      if (res.ok) {
        console.log(`✓ 成功添加: ${rule.title}`);
      } else {
        console.error(`✗ 添加失败: ${rule.title}`, await res.text());
      }
    } catch (error) {
      console.error(`✗ 添加失败: ${rule.title}`, error);
    }
  }
