...
import { Assessment, SourceLocation, BossSummary, DeepDiagnosisResult, BidSuggestion as BidSuggestionType, ObjectiveScoreWarning as ObjectiveScoreWarningType, ServicePeriodAnalysis as ServicePeriodAnalysisType, PreparationChecklist as PreparationChecklistType } from '@/types';
...
import { BossSummary as BossSummaryComponent } from './boss-summary';
import { DeepDiagnosis } from './deep-diagnosis';
import { BidSuggestion } from './bid-suggestion';
import { ObjectiveScoreWarning } from './objective-score-warning';
import { ServicePeriodAnalysis } from './service-period-analysis';
...
interface AssessmentDisplayProps {
  ...
  deepDiagnosis?: DeepDiagnosisResult;
  bidSuggestion?: BidSuggestionType;
  objectiveScoreWarning?: ObjectiveScoreWarningType;
  servicePeriodAnalysis?: ServicePeriodAnalysisType;
  preparationChecklist?: PreparationChecklistType;
}
...
export function AssessmentDisplay({ 
  ...
  deepDiagnosis, 
  bidSuggestion,
  objectiveScoreWarning,
  servicePeriodAnalysis,
  ...
  const handleExportPDF = async () => {
    ...
    setExportStatus('准备PDF数据...');
    
    try {
      setExportProgress(30);
      setExportStatus('生成PDF报告...');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        {(exportingPDF || exportingExcel) && (
          <div className="space-y-2">
            <div className="w-full bg-[#1e1e2e] rounded-full h-2 overflow-hidden">
              <div
                className="h-full rounded-full transition-all duration-300 ease-out bg-gradient-to-r from-[#7c3aed] to-[#06b6d4]"
      ...
      {/* 老板总结 */}
      {bossSummary && (
        <BossSummaryComponent data={bossSummary} isPaid={isPaid} />
      )}

      {/* 客观分风险提示 */}
      {objectiveScoreWarning && objectiveScoreWarning.triggered && (
        <ObjectiveScoreWarning data={objectiveScoreWarning} />
      )}

      {/* 服务期限分析 */}
      {servicePeriodAnalysis && (
        <ServicePeriodAnalysis data={servicePeriodAnalysis} userPlan={isPaid ? 'pro' : 'single'} />
      )}

      {/* 投标建议 */}
      {bidSuggestion && (
        <BidSuggestion data={bidSuggestion} isPaid={isPaid} />
      )}

      {/* 深度诊断 */}
      {deepDiagnosis && (
        <DeepDiagnosis data={deepDiagnosis} isPaid={isPaid} />
      )}

      {/* 准备分工项目包 */}
      {preparationChecklist && (
        <PreparationChecklist data={preparationChecklist} isPaid={isPaid} />
      )}
