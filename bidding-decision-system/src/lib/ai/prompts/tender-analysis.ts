...
**提取规则**：商务评审标准表里的总分 = commercialScore，技术评审标准表里的总分 = technicalScore。如果原文写"商务评审20"，则commercialScore=20；"技术评审60"，则technicalScore=60。不要用固定默认值。
逐字提取并完整拆解：
...
- voidBidConditions: 废标/无效报价条件（在招标文件中搜索所有"废标""无效报价""否决投标""不予受理""按无效处理""其投标无效""视为无效"相关条款，逐条归纳总结为①②③编号列表。每条格式："①[分类如密封/资质/时间/报价/偏离/信用/串标]：简述什么情况会导致废标或无效报价"。不要复制原文长段落，要归纳为核心条件。重复条款只列一次。没有明确后果的条款不要列入。）
...
16. **废标/无效报价条件必须归纳为①②③编号列表**：在全文搜索废标、无效报价相关条款，逐条归纳核心条件（不要复制原文长段落），每条说明什么情况会导致废标或无效报价。每条格式："①[分类]：条件说明"。
