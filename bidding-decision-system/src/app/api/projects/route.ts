...
export async function GET(request: NextRequest) {
  try {
    ...
    const unlocks = await prisma.projectUnlock.findMany({
      where: {
        userId: session.user.id,
    ...
    const unlockSet = new Set(unlocks.map(u => u.projectId));

    // 处理项目数据，添加解锁状态
    const projectsWithStatus = projects.map((project) => {
      const isUnlocked = unlockSet.has(project.id);
      return {
        ...project,
        isUnlocked,
        // hasBidAnalysis 直接从 Project 模型读取
      };
    });
