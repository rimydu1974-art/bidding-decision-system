...
export default function BidAnalyzeContent() {
  ...
  useEffect(() => {
    if (projectId) {
      fetch(`/api/assessments/${projectId}`).then(r => r.json()).then(d => {
        if (d.assessment) setProjectName(d.assessment.projectName || '');
      }).catch(() => {});
    }
  }, [projectId]);
  ...
  useEffect(() => {
    if (analyzing) {
      ...
      progressTimerRef.current = setInterval(() => {
        ...
        const matched = ANALYZE_STAGES.filter((s) => currentPercent >= s.percent);
        if (matched.length > 0) setProgressStage(matched[matched.length - 1].label);
      }, 600);
      return () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      formData.append('projectName', '投标文件上传');
      if (projectId) {
        ...
        <div className="max-w-[800px] mx-auto">
          ...
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-white mb-2">
              {projectId ? `投标文件分析 - ${projectName || '关联项目'}` : '投标文件分析'}
            </h1>
            <p className="text-[#6b7280]">
              {projectId ? '上传投标文件，系统将与招标文件对比生成深度评估' : '上传投标文件，AI 智能评分预测并给出优化建议'}
            </p>
          </div>
          ...
          <div className="glass-card p-6 flex flex-col" style={{ minHeight: '480px' }}>
            ...
            >
              ...
              />
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#06b6d4]/20 to-[#06b6d4]/5 flex items-center justify-center mx-auto mb-4">
            ...
            {files.length > 0 && (
              <div className="mt-4 space-y-2">
                {files.map((f, i) => (
                  ...
                  >
                    <FileText className="w-5 h-5 text-[#22d3ee] flex-shrink-0" />
                ...
                <button
                  ...
                  className={`
                    ...
                    ${files.length === 0
                      ...
                      : 'bg-gradient-to-r from-[#06b6d4] to-[#7c3aed] text-white hover:opacity-90 cursor-pointer'
                    }
                  `}
                >
                  开始AI分析
